/**
 * Unified Visual Builder with Integrated AI
 * 
 * LOCATION: /src/admin/visual-builder/components/UnifiedVisualBuilderPage.tsx
 * 
 * Single interface with:
 * - AI input bar at the top (optional helper, not a separate mode)
 * - Full configuration panels always visible
 * - Product search with cascading filters
 * - Live preview
 * 
 * AI is just a shortcut - you can always configure manually.
 */
...
const CHART_TYPES: { id: VisualizationType; label: string; icon: React.ReactNode; desc: string }[] = [
  ...
  { id: 'treemap', label: 'Treemap', icon: <BarChart3 className="w-5 h-5" />, desc: 'Hierarchical' },
];

const BREAKDOWN_FIELDS = [
  { field: 'origin_state', label: 'Origin State', icon: '📍' },
  { field: 'destination_state', label: 'Destination State', icon: '🎯' },
  { field: 'carrier_name', label: 'Carrier', icon: '🚛' },
  { field: 'mode_name', label: 'Mode', icon: '📦' },
  { field: 'equipment_name', label: 'Equipment', icon: '🚚' },
  { field: 'status_name', label: 'Status', icon: '📋' },
];

const MEASURE_FIELDS = [
  { field: 'retail', label: 'Revenue', icon: '💰', desc: 'Customer charge' },
  { field: 'cost', label: 'Cost', icon: '📉', desc: 'Carrier cost', admin: true },
  { field: 'miles', label: 'Miles', icon: '🛣️', desc: 'Distance' },
  { field: 'total_weight', label: 'Weight', icon: '⚖️', desc: 'Total weight' },
];
...
function AIInputBar() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Call the widget-builder-ai edge function
      ...
      if (result.success) {
        // Apply AI suggestions to builder state
        setVisualization({
        ...
        if (result.filters && result.filters.length > 0) {
          const filterBlock: FilterBlock = {
            ...
            type: 'filter',
            conditions: result.filters.map((f: any) => ({
              field: f.field,
              operator: f.operator,
              value: f.value,
            })),
...
// =============================================================================
// CONFIG PANEL - Chart type, X/Y fields, aggregation
// =============================================================================
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {AGGREGATION_OPTIONS.map(agg => (
            <button
              key={agg.value}
              onClick={() => setVisualization({ aggregation: agg.value as any })}
              className={`
...
function BreakdownValuePicker({ field }: { field: string }) {
  ...
  const existingFilter = state.logicBlocks.find(
    b => b.type === 'filter' && b.conditions.some(c => c.field === field)
  ) as FilterBlock | undefined;

  // Load values when expanded
  useEffect(() => {
  ...
  // Initialize selected from existing filter
  useEffect(() => {
    if (existingFilter) {
  ...
  const updateFilter = (values: string[]) => {
    ...
    if (values.length === 0 || values.length === availableValues.length) {
      // Remove filter if none or all selected
      if (existingFilter) {
