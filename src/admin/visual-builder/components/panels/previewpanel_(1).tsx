...
// Chart components - these would be your actual chart library components
...
export function PreviewPanel() {
  ...
  // Compile current state to execution params
  ...
  const fetchPreviewData = async () => {
    ...
    try {
      ...
      // Apply filters from filterConditions (new format)
...
function ChoroplethPreview({ data, config }: { data: any[]; config: any }) {
  // Simplified US states SVG preview
  return (
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
        ...
        <YAxis tick={{ fontSize: 11, fill: '#64748b' }} label={{ value: 'Frequency', angle: -90, position: 'insideLeft' }} />
        <Tooltip />
...
function processDataForVisualization(rawData: any[], config: any): any[] {
  const { type, xField, yField, groupBy, aggregation, geo, flow } = config;
