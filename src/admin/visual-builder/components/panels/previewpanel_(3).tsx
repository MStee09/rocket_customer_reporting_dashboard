...
import {
  ...
  PieChart,
  Table2,
  Map as MapIcon,  // Renamed to avoid shadowing native Map
  TrendingUp,
  Database,
} from 'lucide-react';
...
export function PreviewPanel() {
  ...
  const fetchPreviewData = async () => {
    ...
    try {
      ...
      // Apply user-defined filter conditions from logic blocks
...
function processDataForVisualization(rawData: any[], config: any): any[] {
  ...
  if (type === 'flow' && flow?.originField && flow?.destinationField) {
    // Use Object instead of Map to avoid the shadowing issue
...
function groupAndAggregate(
  ...
  // Use Object instead of Map to avoid shadowing issue
