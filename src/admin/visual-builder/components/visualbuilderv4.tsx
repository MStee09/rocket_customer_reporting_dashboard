...
export function VisualBuilderV4() {
  ...
  const { user, isAdmin, effectiveCustomerId, customers } = useAuth();
    ...
    for (const step of reasoning) {
      if (step.type === 'tool_call' && step.toolName === 'search_text') {
        try {
          ...
          if (match) cfg.searchTerms.push(match[1]);
        } catch {}
      }
      ...
      if (step.type === 'tool_result' && step.content) {
        try {
          ...
          if (content.includes('ILIKE') || content.includes('ilike')) {
            ...
            if (ilikeMatches) {
              ilikeMatches.forEach((match: string) => {
                const parts = match.match(/(\w+)\s+ILIKE\s+'([^']+)'/i);
                if (parts) {
                  cfg.filters.push({ field: parts[1], operator: 'ilike', value: parts[2] });
                }
              });
            }
          }
          if (content.includes('avg(')) cfg.aggregation = 'avg';
          else if (content.includes('sum(')) cfg.aggregation = 'sum';
          else if (content.includes('count(')) cfg.aggregation = 'count';

          if (content.includes('group_by')) {
            const gbMatch = content.match(/group_by['":\s]+(\w+)/);
            if (gbMatch) cfg.xAxis = gbMatch[1];
          }
        } catch {}
      }
    }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-4">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-blue-600 to-teal-600 text-white rounded-lg hover:from-blue-700 hover:to-teal-700 disabled:from-slate-400 disabled:to-slate-400 font-semibold text-sm"
      >
        {isPublishing ? (
          <>
