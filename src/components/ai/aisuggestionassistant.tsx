...
// Known field mappings for logistics domain
const FIELD_HINTS: Record<string, string[]> = {
  product: ['shipment_description', 'po_number', 'reference_number', 'item_description'],
  cost: ['retail', 'cost', 'total_cost', 'freight_cost'],
  carrier: ['carrier_name', 'carrier_id', 'scac'],
  location: ['origin_state', 'destination_state', 'origin_city', 'destination_city'],
  date: ['created_date', 'pickup_date', 'delivery_date'],
  customer: ['customer_name', 'customer_id'],
  weight: ['total_weight', 'billed_weight'],
  mode: ['mode_name', 'service_type'],
};
...
export function AISuggestionAssistant() {
  ...
  const applyAllSteps = () => {
    ...
    // Add AI block if complex logic needed
 ...
 * This is a simplified version - in production you'd call your AI service
 */
function parseUserIntent(prompt: string): AISuggestion {
  ...
  if (lower.includes('cost') || lower.includes('spend') || lower.includes('price')) {
    ...
    yField = '';  // Will use count
    aggregation = 'count';
  }

  // Detect product filtering (your use case!)
  ...
  if (productMatches.length > 0) {
    // For OR logic with contains, we need an AI block or special operator
    aiPrompt = `Filter shipments where description contains any of: ${productMatches.map(p => `"${p}"`).join(', ')}`;
    
    // Also create the filter with contains_any operator
    filters.push({
      field: 'shipment_description',
      operator: 'contains_any' as any, // New operator we'll add
