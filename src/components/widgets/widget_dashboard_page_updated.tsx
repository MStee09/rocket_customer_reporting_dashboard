// =============================================================================
// FILE: src/pages/DashboardPage.tsx (UPDATED for hybrid hover + edit mode)
// =============================================================================
// 
// KEY CHANGES:
// 1. Hover drag reordering auto-saves (with debounce)
// 2. Edit mode changes require explicit "Done" click
// 3. Visual feedback for auto-save
//
// =============================================================================
...
import { InlineEditToolbar } from '../components/dashboard/InlineEditToolbar';
...
import { clampWidgetSize } from '../config/widgetConstraints';
...
export function DashboardPage() {
  ...
  // Track if layout was changed via hover drag (for auto-save)
  const [hoverDragPending, setHoverDragPending] = useState(false);
  const debouncedLayout = useDebounce(localLayout, 1000); // 1 second debounce for auto-save
  const lastSavedLayoutRef = useRef<string[]>(layout);

  // Convert stored sizes to WidgetSizeValue format
  const convertSizes = useCallback((sizes: Record<string, any>): Record<string, WidgetSizeValue> => {
    const converted: Record<string, WidgetSizeValue> = {};
    Object.entries(sizes).forEach(([key, value]) => {
      if (value === 'default' || value === 1) converted[key] = 1;
      else if (value === 'large' || value === 2) converted[key] = 2;
  ...
  // Sync local state with saved state when not editing
  useEffect(() => {
  ...
  // Auto-save when layout changes via hover drag (debounced)
  useEffect(() => {
    // Only auto-save if:
    // 1. Not in edit mode (edit mode requires explicit save)
    // 2. Layout actually changed from last saved
    // 3. Hover drag is pending
    if (
  ...
  // Load available widgets for gallery
  useEffect(() => {
  ...
  // Load AI widgets from localStorage
  useEffect(() => {
    const loadAIWidgets = () => {
  ...
  // Handle addWidget URL parameter
  useEffect(() => {
  ...
  const handleWidgetSizeChange = (widgetId: string, size: WidgetSizeValue) => {
    const widget = widgetLibrary[widgetId] || customWidgets[widgetId];
    if (widget) {
      const clampedSize = clampWidgetSize(size, widgetId, widget.type);
  ...
  const handleReorder = (newOrder: string[]) => {
    ...
    if (editMode.state.isEditing) {
      // In edit mode: track as pending change (requires explicit save)
      editMode.setPendingChanges(true);
    } else {
      // In hover mode: mark for auto-save
      setHoverDragPending(true);
    }
  };

  const handleAddWidget = (widgetId: string, size: WidgetSizeValue) => {
  ...
  const handleSaveChanges = async () => {
    const sizesToSave: Record<string, any> = {};
    Object.entries(localSizes).forEach(([key, value]) => {
      sizesToSave[key] = value;
    });
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="container mx-auto px-6 py-8 max-w-[1600px]">
        ...
        <WidgetGrid
          ...
          allowHoverDrag={true} // Enable hover drag for quick reordering
        />
