...
export default function ColumnFilterSection({
  ...
  const getSortIcon = () => {
    ...
    if (sort.direction === 'asc') return <ArrowUp className="w-4 h-4 text-blue-600" />;
    ...
    <div className="mt-2 pt-2 border-t border-gray-100">
      {!isExpanded && filters.length === 0 && (
        <div className="flex items-center gap-2">
          <button onClick={addFilter} className="flex items-center gap-1.5 px-2 py-1 text-xs text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors">
            <Filter className="w-3 h-3" />
            Add Filter
          </button>
          <div className="h-4 w-px bg-gray-200" />
          <button onClick={cycleSortDirection} className="flex items-center gap-1.5 px-2 py-1 text-xs text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors">
            {getSortIcon()}
      ...
      {(isExpanded || filters.length > 0) && (
        <div className="space-y-2">
          {filters.map((filter) => {
            ...
            return (
              <div key={filter.id} className={`flex items-start gap-2 p-2 rounded-lg ${filter.enabled ? 'bg-blue-50 border border-blue-200' : 'bg-gray-50 border border-gray-200'}`}>
                <button onClick={() => toggleFilterEnabled(filter.id)} className={`flex-shrink-0 mt-1 ${filter.enabled ? 'text-blue-600' : 'text-gray-400'}`} title={filter.enabled ? 'Disable filter' : 'Enable filter'}>
                ...
                <select
                  ...
                  className={`px-2 py-1.5 text-sm border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${filter.enabled ? 'border-gray-300 bg-white' : 'border-gray-200 bg-gray-100 text-gray-500'}`}
                >
                  {conditions.map(cond => (
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="h-4 w-px bg-gray-200" />
            <button onClick={cycleSortDirection} className={`flex items-center gap-1.5 px-2 py-1 text-xs rounded transition-colors ${sort && sort.direction !== 'none' ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:text-blue-600 hover:bg-blue-50'}`}>
              {getSortIcon()}
              <span>Sort: {getSortLabel()}</span>
            </button>
          </div>
