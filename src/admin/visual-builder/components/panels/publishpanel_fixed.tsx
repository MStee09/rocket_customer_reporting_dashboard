 ...
 * 
 * FIX: Changed 'customers' to 'customer' table and 'id' to 'customer_id'
 */
...
export function PublishPanel() {
  ...
  // Use customers from AuthContext if available, otherwise load them
  useEffect(() => {
    if (state.publish.scope === 'customer') {
      if (authCustomers && authCustomers.length > 0) {
        // Use customers from AuthContext
  ...
  const loadCustomers = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('customer')  // FIXED: was 'customers'
        .select('customer_id, company_name')  // FIXED: was 'id, company_name'
        .eq('is_active', true)
        .order('company_name');
