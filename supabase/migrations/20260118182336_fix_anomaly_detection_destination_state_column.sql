/*
  # Fix Anomaly Detection - Use correct column name destination_state
  
  1. Changes
    - Update new lane detection to use `destination_state` instead of `dest_state`
    - This matches the actual column name in shipment_report_view
  
  2. Impact
    - New lane detection will now work correctly
    - Will properly identify first-time shipping lanes
*/
...
BEGIN
  ...
  -- ========================================================================
  -- NEW LANE DETECTION (using correct column: destination_state)
  -- ========================================================================
  IF v_config.detect_lane_anomalies THEN
    ...
    SELECT
      ...
      format('First shipments detected on lane: %s → %s', origin_state, destination_state),
      'lane_count',
      1,
      0,
      100,
    ...
    WHERE NOT EXISTS (
      ...
      WHERE customer_id = p_customer_id 
        ...
        AND affected_value = new_lanes.origin_state || ' → ' || new_lanes.destination_state
        AND detection_date > NOW() - INTERVAL '30 days'
    )
    LIMIT 3;
...
GRANT EXECUTE ON FUNCTION run_anomaly_scan_for_customer TO service_role, authenticated;
