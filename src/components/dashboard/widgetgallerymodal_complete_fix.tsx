...
interface WidgetGalleryModalProps {
  ...
  isAdmin: boolean;
  customerId?: number;  // NEW: Pass customer ID to load customer-specific widgets
}

interface WidgetWithMeta extends WidgetDefinition {
  isNew?: boolean;
  isRecommended?: boolean;
  isCustom?: boolean;  // NEW: Flag for custom widgets
}
...
const typeIcons: Record<string, typeof Star> = {
  ...
  table: Table,
  map: Map,
  custom: Paintbrush,  // NEW: Icon for custom widgets
};
...
export function WidgetGalleryModal({
  ...
  const [expandedCustom, setExpandedCustom] = useState(true);  // NEW
  const [showAllWidgets, setShowAllWidgets] = useState(false);
  
  // NEW: State for custom widgets
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setExpandedNew(true);
      setExpandedCustom(true);  // NEW
      setShowAllWidgets(false);
    }
  }, [isOpen]);

  // NEW: Load custom widgets when modal opens
  useEffect(() => {
    const loadCustomWidgetsForGallery = async () => {
      ...
      try {
        ...
        const widgetDefs: WidgetWithMeta[] = customWidgets.map(cw => ({
          ...
          category: 'breakdown' as WidgetCategory, // Default category
          ...
          isNew: false,
          isRecommended: false,
          calculate: async () => null, // Custom widgets use different data loading
        }));
  ...
  // MODIFIED: Include custom widgets in allWidgets
  const allWidgets = useMemo((): WidgetWithMeta[] => {
    ...
    // Combine library widgets with custom widgets
  ...
  // NEW: Filter for custom widgets
  const customWidgets = useMemo(() =>
    searchFilteredWidgets.filter(w => w.isCustom && !currentWidgets.includes(w.id)),
    [searchFilteredWidgets, currentWidgets]
  );

  // Filter for standard library widgets (excluding custom)
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-auto p-4 space-y-6">
              ...
              {/* NEW: Custom Widgets Section - Show first! */}
