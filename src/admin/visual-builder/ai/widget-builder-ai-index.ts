/**
 * Widget Builder AI Edge Function
 * 
 * LOCATION: supabase/functions/widget-builder-ai/index.ts
 * 
 * This uses the SAME MCP pattern as the investigate function to provide
 * real schema-aware suggestions for the Visual Widget Builder.
 * 
 * Instead of pattern matching, it:
 * 1. Discovers what tables/fields exist
 * 2. Searches for where products/carriers/terms exist
 * 3. Returns a config that will actually work
 */
...
// MCP Tools - subset focused on schema discovery
...
// Note: Function names match the database functions in 20260109045954_create_mcp_functions_complete.sql
async function executeMCPTool(
  ...
  switch (toolName) {
    case "discover_tables": {
      // mcp_get_tables(p_category, p_include_row_counts)
    ...
    case "discover_fields": {
      // mcp_get_fields(p_table_name, p_include_samples, p_admin_mode)
      const { data, error } = await supabase.rpc('mcp_get_fields', {
        p_table_name: input.table_name,
        p_include_samples: true,
        p_admin_mode: true // Widget builder is admin-only anyway
      });
      if (error) throw error;
      return data;
    }
    ...
    case "discover_joins": {
      // mcp_get_table_joins(p_table_name)
    ...
    case "search_text": {
      // mcp_search_text(p_search_query, p_customer_id, p_is_admin, p_match_type, p_limit)
      // Note: For widget builder, we search across ALL customers (admin mode)
      const { data, error } = await supabase.rpc('mcp_search_text', {
        p_search_query: input.query,
        p_customer_id: 0, // Not used when p_is_admin is true
        p_is_admin: true,
...
Return your final recommendation as a JSON code block.`
    ...
    // Parse the final answer to extract JSON config
...
function parseAIResponse(answer: string, reasoning: string[]): WidgetSuggestion {
  ...
  // Build the suggestion from parsed config or defaults
  return {
    success: true,
...
function extractSummary(text: string): string {
  // Extract first sentence or meaningful summary
