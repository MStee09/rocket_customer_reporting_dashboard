/**
 * AI Suggestion Assistant - MCP-Aware Version (Simplified)
 *
 * LOCATION: /src/admin/visual-builder/components/AISuggestionAssistant.tsx
 *
 * This version:
 * - Uses local parsing for speed (no Edge Function call for suggestions)
 * - Generates MCP query config for PreviewPanel to use
 * - Knows about shipment_item.description for product filtering
 */
    ...
    switch (step.action.type) {
      ...
      case 'add_filter':
        addLogicBlock({
          ...
          mcpQuery: step.action.payload.mcpQuery, // Pass MCP config
        });
...
/**
 * Parse user intent and generate MCP-aware suggestions
 */
function parseUserIntent(prompt: string): AISuggestion {
  ...
  // Build filters and MCP query for products
  const filters: FilterCondition[] = [];
  let mcpQuery: any = null;
  let aiPrompt = '';

  if (uniqueProducts.length > 0) {
    // Products are in shipment_item.description - need MCP query
    info = `Product filtering will use MCP to search shipment_item.description for: ${uniqueProducts.join(', ')}`;
    
    // Set xField to group by product
    if (!xField) {
      xField = 'description';
    }

    // Build MCP query config for PreviewPanel
    mcpQuery = {
      base_table: 'shipment',
    ...
    // Build filter condition for logic block
    filters.push({
      field: 'shipment_item.description',
