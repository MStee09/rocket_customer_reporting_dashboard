...
interface RequestBody {
  ...
  customerId: string;
  isAdmin?: boolean; // DEPRECATED: Ignored - admin status verified server-side from user_roles table
...
/**
 * SECURITY: Server-side admin role verification
 * Never trust client-sent isAdmin flag - always verify against database
 */
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // NOTE: We intentionally ignore body.isAdmin - it's verified server-side below
    ...
    // SECURITY: Server-side admin verification - NEVER trust client's isAdmin claim
    let isAdmin = false;
    if (userId) {
