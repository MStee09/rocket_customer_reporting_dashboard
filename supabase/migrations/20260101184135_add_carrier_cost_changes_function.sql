/*
  # Add Carrier Cost Changes Function

  1. New Functions
    - `get_carrier_cost_changes(p_customer_id TEXT, p_start_date DATE, p_end_date DATE)` - Analyzes carrier cost changes between periods

  2. Purpose
    - Identifies carriers with significant cost changes (>5%)
    - Returns top 10 carriers ordered by absolute change percentage
    - Used by ProactiveInsightsCard for anomaly detection
  ...
  FROM (
    ...
    FROM (
      ...
      WHERE customer_id = p_customer_id::INTEGER
        ...
        AND carrier_name IS NOT NULL AND retail IS NOT NULL
