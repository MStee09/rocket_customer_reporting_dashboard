    ...
    queryFn: async () => {
      if (isCustomWidget) {
        ...
        if (isVisualBuilderWidget) {
          ...
          if (aiConfig?.searchTerms && aiConfig.searchTerms.length > 0) {
            aiConfig.searchTerms.forEach((term: string) => {
              queryFilters.push({
                ...
                operator: 'ilike',
                value: `%${term}%`  // Add wildcards for ILIKE
              });
            });
          }
          ...
          // Handle various response structures from Supabase RPC
          // The mcp_aggregate function returns JSONB like: {"data": [...]}
          // But Supabase RPC wraps it, so we might get: {data: {data: [...]}}
          let rows: any[] = [];
