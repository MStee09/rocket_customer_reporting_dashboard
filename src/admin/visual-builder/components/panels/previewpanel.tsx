/**
 * Preview Panel - MCP-Enabled Version
 *
 * LOCATION: /src/admin/visual-builder/components/panels/PreviewPanel.tsx
 *
 * Changes from original:
 * - Detects when MCP query is needed (product filters)
 * - Calls generate-report Edge Function for MCP queries
...
export function PreviewPanel() {
  ...
  const needsMCPQuery = (): boolean => {
    ...
    for (const block of state.logicBlocks) {
      // Check for explicit MCP query
      if ((block as any).mcpQuery) return true;
      
      // Check for filters on joined tables (shipment_item, carrier, etc.)
  ...
  const fetchPreviewDataWithMCP = async () => {
    ...
    try {
      // Find the MCP query from logic blocks
      let mcpQuery: any = null;
      let productFilters: string[] = [];

      for (const block of state.logicBlocks || []) {
        if ((block as any).mcpQuery) {
          mcpQuery = (block as any).mcpQuery;
        }
        if (block.type === 'filter' && block.conditions) {
          for (const condition of block.conditions) {
...
function processDataForVisualization(rawData: any[], config: any): any[] {
  ...
  // If no xField, try to extract from description for product grouping
...
function groupAndAggregate(
  ...
  for (const row of data) {
    ...
    // For product descriptions, extract product type
    if (groupField === 'description' && key.length > 30) {
      // Extract first part before "/" or special chars
