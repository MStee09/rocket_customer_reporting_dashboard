/*
  # Updated Pulse Dashboard RPC Functions (v2)

  1. Changes
    - Improved error handling in get_pulse_performance_summary
    - Better date coalescing using pickup_date with fallback to created_at
    - Added explicit search_path for security

  2. Functions Updated
    - get_pulse_executive_metrics - Executive metrics with trend comparison
    - get_pulse_spend_trend - Daily spend for area chart
    - get_pulse_top_carriers - Top carriers by volume
    - get_pulse_performance_summary - Volume breakdown and top lane
*/
...
BEGIN
  ...
  WHERE s.customer_id = p_customer_id
    AND COALESCE(s.pickup_date::DATE, s.created_at::DATE) >= p_start_date
    AND COALESCE(s.pickup_date::DATE, s.created_at::DATE) <= p_end_date;
  ...
  FROM shipment s
  WHERE s.customer_id = p_customer_id
    AND COALESCE(s.pickup_date::DATE, s.created_at::DATE) >= prev_start
    AND COALESCE(s.pickup_date::DATE, s.created_at::DATE) <= prev_end;
...
BEGIN
  RETURN QUERY
  SELECT 
    COALESCE(s.pickup_date::DATE, s.created_at::DATE) as date,
  ...
  WHERE s.customer_id = p_customer_id
    AND COALESCE(s.pickup_date::DATE, s.created_at::DATE) >= p_start_date
    AND COALESCE(s.pickup_date::DATE, s.created_at::DATE) <= p_end_date
  GROUP BY COALESCE(s.pickup_date::DATE, s.created_at::DATE)
  ORDER BY COALESCE(s.pickup_date::DATE, s.created_at::DATE);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;
