...
export function CostPerStateMap({ data, isLoading }: CostPerStateMapProps) {
  ...
  const getLabelFontSize = (isSmallRegion: boolean) => {
    if (zoom > 2) return 11;
    if (zoom > 1.5) return 10;
    if (isSmallRegion) return 7;
    return 8;
  };
          ...
          >
            ...
            {showLabels &&
              Object.entries(ALL_REGION_COORDINATES).map(([code, coordinates]) => {
                ...
                const fontSize = getLabelFontSize(isSmallRegion);
...
|---------|---------------|
| **Label font size** | 8px default, 10-11px when zoomed > 1.5x |
| **Label font weight** | 600 (semibold) |
| **Label color** | `#ffffff` with shadow on colored regions, `#94a3b8` on gray |
| **Label text shadow** | `0 1px 2px rgba(0,0,0,0.6)` on colored regions only |
| **Small region labels** | Hide when zoom < 1.5 |
