/**
 * Preview Panel - MCP-Enabled Version (Fixed)
 *
 * LOCATION: /src/admin/visual-builder/components/panels/PreviewPanel.tsx
 *
 * Fixed:
 * - Gets customer ID from useAuth() properly
 * - Handles OR filters correctly for multiple products
...
export function PreviewPanel() {
  ...
  // Check if any logic block requires MCP (has mcpQuery or filters on joined tables)
  ...
  const fetchPreviewDataWithMCP = async () => {
    ...
    try {
      ...
      // For multiple products, we need to use OR logic
      // The MCP function treats multiple filters as AND, so we need a different approach
      // Let's query all data and filter client-side for simplicity
      ...
      const { data: result, error: rpcError } = await supabase.rpc('mcp_query_with_join', {
        ...
        p_filters: [], // Get all data, filter client-side for OR logic
        p_group_by: null,
        p_aggregations: null,
 ...
 * Groups by product description and aggregates values
 */
function processDataForVisualization(rawData: any[], config: any, productFilters: string[]): any[] {
  ...
  // For MCP product queries - group by product category
    ...
    for (const row of rawData) {
      ...
      for (const product of productFilters) {
        if (desc.includes(product.toLowerCase())) {
          ...
          break; // Only count in first matching product
        }
      }
    }
