/**
 * Visual Builder V5 - PRODUCTION READY
 * 
 * CRITICAL FIXES:
 * 1. SECURITY: Admin-only columns (cost, margin, carrier pay) hidden from customers
 * 2. COLUMNS: X-Axis and Y-Axis show the SAME columns (no arbitrary filtering)
 * 3. AI: Better prompt engineering to get correct grouping and aggregation
 * 4. DATA: Proper validation and formatting
 */
...
export function VisualBuilderV5() {
...
Return a clear visualization with properly aggregated data.`;
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      setPublishResult({ success: true, message: `Widget "${config.name}" published successfully!` });
    } catch (err) {
      ...
      {hasData && !loading && (
        <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
          ...
          <div className="p-4 border-b border-slate-100">
            <h5 className="text-xs font-semibold text-slate-500 uppercase mb-2">Data Preview</h5>
...
function PublishSection({
  ...
  // Check if widget uses admin-only data
  const usesAdminData = config.metricColumn && ADMIN_ONLY_COLUMNS.has(config.metricColumn);
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-4">
      ...
      {/* Visibility - restrict options if using admin data */}
      <div>
        ...
        {usesAdminData && (
          <div className="mb-2 p-2 bg-amber-50 border border-amber-200 rounded flex items-start gap-2">
            ...
            <p className="text-xs text-amber-800">
              This widget uses sensitive data (cost/margin). It can only be visible to admins.
            </p>
          </div>
        )}
        ...
        <div className="space-y-2">
          <button
            ...
            className={`w-full flex items-center gap-2 p-2 rounded-lg border text-left text-sm ${
              visibility === 'admin_only' ? 'border-blue-500 bg-blue-50' : 'border-slate-200'
            }`}
          >
            <Shield className={`w-4 h-4 ${visibility === 'admin_only' ? 'text-blue-600' : 'text-slate-400'}`} />
            <span className={visibility === 'admin_only' ? 'text-blue-900 font-medium' : 'text-slate-700'}>Admin Only</span>
          </button>
          ...
          {!usesAdminData && (
            <>
              <button
                ...
                className={`w-full flex items-center gap-2 p-2 rounded-lg border text-left text-sm ${
                  visibility === 'all_customers' ? 'border-blue-500 bg-blue-50' : 'border-slate-200'
                }`}
              >
                <Users className={`w-4 h-4 ${visibility === 'all_customers' ? 'text-blue-600' : 'text-slate-400'}`} />
                <span className={visibility === 'all_customers' ? 'text-blue-900 font-medium' : 'text-slate-700'}>All Customers</span>
              </button>

              <button
                onClick={() => setVisibility('private')}
                className={`w-full flex items-center gap-2 p-2 rounded-lg border text-left text-sm ${
                  visibility === 'private' ? 'border-blue-500 bg-blue-50' : 'border-slate-200'
                }`}
              >
                <Lock className={`w-4 h-4 ${visibility === 'private' ? 'text-blue-600' : 'text-slate-400'}`} />
                <span className={visibility === 'private' ? 'text-blue-900 font-medium' : 'text-slate-700'}>Private</span>
              </button>
            </>
          )}
        </div>
...
export default VisualBuilderV5;
