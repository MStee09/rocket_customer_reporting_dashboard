...
function FiltersPanel() {
  ...
  // Check both old field name (description) and new field name (item_descriptions)
  ...
  const handleProductSearch = async () => {
    ...
    try {
      // Search in shipment_report_view using item_descriptions field
      const { data } = await supabase
        .from('shipment_report_view')
      ...
      if (data) {
        ...
        for (const row of data) {
          if (row.item_descriptions) {
            // Extract matching terms from the concatenated descriptions
  ...
  const toggleProduct = (value: string) => {
    ...
    const condition: FilterCondition = {
      field: 'item_descriptions',  // Use the new field from shipment_report_view
