...
// Helper to load Visual Builder widget from storage
...
export async function executeWidget(
  ...
  // First check for standard widget
  const widgetDef = getWidgetById(widgetId);

  // If not found in registry, try loading as Visual Builder widget
  if (!widgetDef) {
    ...
    if (customWidget && customWidget.dataSource?.groupByColumn) {
      ...
      // Execute Visual Builder widget query
      ...
      // Add product filters from aiConfig
      ...
      // Now fetch detailed row data for the table view
      // Query the underlying shipments that match the filters
      let detailQuery = supabase
        .from(isProductQuery ? 'shipment_item' : 'shipment')
        .select(isProductQuery 
          ? 'load_id, description, quantity, weight, retail, cost'
          : 'load_id, reference_number, pickup_date, delivery_date, retail, cost, origin_city, origin_state, dest_city, dest_state'
        )
        .gte('pickup_date', dateRange.start)
        .lte('pickup_date', dateRange.end)
        .limit(500);
      ...
      // Add product filters for detail query
      ...
      const columns = isProductQuery 
        ...
        : [
            ...
            { key: 'pickup_date', label: 'Pickup Date', type: 'date' },
            { key: 'delivery_date', label: 'Delivery Date', type: 'date' },
            { key: 'origin_city', label: 'Origin', type: 'string' },
            { key: 'dest_city', label: 'Destination', type: 'string' },
            { key: 'retail', label: 'Cost', type: 'currency' },
          ];
      ...
      return {
        ...
        widgetData: {
          ...
          data: aggRows.map((row: any) => {
            const labelKey = Object.keys(row).find(k => typeof row[k] === 'string') || groupByField;
            const valueKey = Object.keys(row).find(k => k.includes(aggregation || 'avg') || k === 'value' || typeof row[k] === 'number');
            return { name: row[labelKey], value: row[valueKey] || row.value };
          }),
        },
        tableData,
