/**
 * BuilderContext.tsx - State Management for Visual Builder
 * 
 * INCREMENTAL UPDATE: Added customerScope support
 * 
 * LOCATION: /src/admin/visual-builder/components/BuilderContext.tsx
 */
...
type BuilderAction =
  ...
  | { type: 'SET_CUSTOMER_SCOPE'; payload: CustomerScope }  // NEW
  | { type: 'MARK_DIRTY' }
  | { type: 'MARK_CLEAN' }
...
function builderReducer(
  ...
  switch (action.type) {
    ...
    // NEW: Customer scope handler
    case 'SET_CUSTOMER_SCOPE':
      return {
        ...state,
        customerScope: action.payload,
        // Auto-update publish scope when customer is selected
...
interface BuilderContextValue {
  ...
  setCustomerScope: (scope: CustomerScope) => void;  // NEW
  reset: () => void;
