...
interface ConversationMessage {
  ...
  timestamp: Date;
  feedbackRating?: 'good' | 'bad' | null; // Track if user rated this
  relatedQuestion?: string; // The user question this answers
}
...
function RadarChartViz({ viz }: { viz: Visualization }) {
  ...
  const normalizedData = chartData.map(d => ({
    subject: d.label || d.name || 'Unknown',
    value: d.value,
    fullMark: Math.max(...chartData.map(item => item.value)) * 1.2
  }));
    ...
    <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 my-3">
      ...
      <ResponsiveContainer width="100%" height={280}>
        <RadarChart data={normalizedData}>
          ...
          />
          <Tooltip
            formatter={(value: number) => [formatValue(value, viz.data.format), 'Value']}
            contentStyle={{ borderRadius: '8px', border: '1px solid #e2e8f0', background: '#fff' }}
          />
        </RadarChart>
      </ResponsiveContainer>
...
export function InvestigatorUnified({
  ...
  const handleFeedback = useCallback(async (messageId: string, rating: 'good' | 'bad', userFeedback?: string) => {
    // Find the message and its related question
    ...
    try {
      const { error } = await supabase.from('ai_feedback').insert({
        ...
        user_id: userId,
        conversation_id: conversation[0]?.id, // Use first message id as conversation id
        ...
        tool_calls: message.reasoning?.filter(r => r.type === 'tool_call').map(r => r.toolName) || [],
