/**
 * PreviewPanel - DEBUG VERSION
 * 
 * LOCATION: src/admin/visual-builder/components/panels/PreviewPanel.tsx
 * 
 * This version has extensive debugging to find why 0 rows are returned.
 */
...
export function PreviewPanel() {
  ...
  const fetchPreviewData = async () => {
    ...
    try {
      // STEP 1: Check if shipment_item table exists and has data
      ...
      // STEP 3: If we need product filter, try the join
      if (needsJoin && (itemCount ?? 0) > 0) {
        addDebug('Step 3: Attempting join query...');

        // Try a simple join first
      ...
      // STEP 4: Fall back to shipment_report_view
...
function processDataForVisualization(rawData: any[], config: any, productFilters: string[]): any[] {
  const { type, xField, yField, aggregation } = config;
