...
async function loadVisualBuilderWidget(widgetId: string, customerId?: number): Promise<LoadedWidget | null> {
  // First try the provided customerId path
  if (customerId) {
  ...
  // If not found yet, search through all customer folders
...
export async function executeWidget(
  ...
  if (!widgetDef) {
    ...
    if (loadedWidgetResult && loadedWidgetResult.widget?.dataSource?.groupByColumn) {
      ...
      // Use the owner customer ID if available (the customer folder where widget was found)
      ...
      // Handle both field names (secondaryGroupBy and secondaryGroupByColumn)
      ...
      if (isMultiDimension) {
        ...
        // FIRST: Fetch the detail rows with addresses (since mcp_aggregate can't join shipment_item with shipment_address)
        let detailRows: Record<string, unknown>[] = [];
        
        if (isProductQuery) {
          // First, get valid load_ids from shipment table for this customer and date range
          ...
          if (validLoadIds.length === 0) {
            ...
            // Build description filter for search terms - wrap in parentheses for proper grouping
            // We need to filter by: load_id IN (validLoadIds) AND (description ILIKE term1 OR description ILIKE term2 ...)
            ...
            // Query shipment_item directly - first filter by load_ids, then by description
            // Note: Supabase .in() and .or() combine with AND logic
          ...
          // If we have a secondary grouping like origin_state, we need to fetch addresses
          if (actualSecondaryGroupBy === 'origin_state' || actualSecondaryGroupBy === 'destination_state') {
            ...
            if (loadIds.length > 0) {
              ...
              // Build a lookup map for addresses
              ...
              // Merge address data into detail rows
          ...
          // Fetch addresses for origin_state/destination_state
          if (actualSecondaryGroupBy === 'origin_state' || actualSecondaryGroupBy === 'destination_state') {
        ...
        // Now compute chart data by aggregating the detail rows (since mcp_aggregate can't join shipment_item with shipment_address for origin_state)
        ...
        // Group detail rows by product search term and secondary group
        ...
        // Convert aggregation map to chart data
        ...
        // Build columns with load_id first, then include the secondary group column
      ...
      // For shipment_item queries, don't add pickup_date filters (it's on shipment table)
      const queryFilters: Array<{ field: string; operator: string; value: string }> = isProductQuery
        ? []  // No date filters for shipment_item - they're applied via the get_shipment_items_with_dates RPC
        : [
