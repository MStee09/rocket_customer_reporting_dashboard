...
export function CarrierAnalyticsSection({ 
  ...
  async function loadCarrierData() {
    if (!customerId) {
      console.log('CarrierAnalyticsSection: No customerId provided');
      setLoading(false);
      return;
    }

    console.log('CarrierAnalyticsSection: Loading data for customer', customerId, 'from', startDate, 'to', endDate);
    setLoading(true);

    try {
      // Use shipment_report_view like CarriersPage does
      // This view already has carrier_name joined
      ...
      console.log('CarrierAnalyticsSection: Got', currentData?.length || 0, 'shipments for current period');
      ...
      console.log('CarrierAnalyticsSection: Processed', carriersWithTrends.length, 'carriers');
