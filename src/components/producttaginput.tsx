/**
 * ProductTagInput - User-friendly product filter input
 * 
 * Generates proper FilterBlock entries for product description searches.
 * Shows match counts for each tag.
 * 
 * LOCATION: /src/admin/visual-builder/components/ProductTagInput.tsx
 */
  ...
  for (const block of logicBlocks) {
    if (block.type === 'filter' && block.enabled) {
      for (const condition of block.conditions || []) {
        if (condition.field === 'description' && condition.operator === 'contains_any') {
          if (Array.isArray(condition.value)) {
            tags.push(...condition.value);
          }
        } else if (condition.field === 'description' && condition.operator === 'contains') {
          tags.push(String(condition.value));
        }
        // Also check item_descriptions field
...
export function ProductTagInput() {
      ...
      try {
        ...
        (data || []).forEach(row => {
          if (row.item_descriptions) {
            // Simple extraction - add matching descriptions
  ...
  // Fetch match counts for given terms
  ...
  // Add a new tag and update logicBlocks
  ...
  // Remove a tag and update logicBlocks
