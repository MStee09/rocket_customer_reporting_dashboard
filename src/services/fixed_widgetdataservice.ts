...
export async function executeWidget(
  ...
  try {
    // Call calculate with parameters that work for both widgetLibrary and customerWidgets
    const widgetData = await widgetDef.calculate({
      supabase,
      dateRange,
      // For customerWidgets (uses customerId)
      customerId: customerIdNum,
      // For widgetLibrary (uses effectiveCustomerIds array)
