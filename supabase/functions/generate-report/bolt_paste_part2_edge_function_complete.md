# AI Usage Tracking System - Part 2: COMPLETE Edge Function

## Overview

This is the **COMPLETE** replacement for `supabase/functions/generate-report/index.ts`.

Key changes from original:
1. Added `logUsage()` function to track by actual user
2. Added `userId` and `userEmail` to request body interface
3. Logs usage after each successful/failed request
4. Returns usage info (tokens, costs) in response

---

## FULL FILE - Replace `supabase/functions/generate-report/index.ts`
...
interface RequestBody {
  ...
  customerName?: string;
  useTools?: boolean;
  // NEW: User info for usage tracking
  userId?: string;
  userEmail?: string;
  sessionId?: string;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Get user info from request body (passed from frontend)
    userId = body.userId;
    ...
    if (useTools) {
      ...
      // Legacy audit log (keep for backward compatibility)
      try {
...
1. **Added `logUsage()` function** (lines 95-130) - Calls the `log_ai_usage` RPC function
2. **Added user fields to RequestBody** (lines 28-30) - `userId`, `userEmail`, `sessionId`
3. **Extract user info in handler** (lines 530-535) - Gets user info from request body
4. **Log after tool mode** (lines 680-693) - Logs successful tool-mode requests
5. **Log after legacy mode** (lines 760-773) - Logs successful legacy-mode requests
6. **Log errors** (lines 815-826) - Logs failed requests
7. **Return usage in response** - Both modes now return `usage` object with tokens and costs

The file is complete and ready to replace your existing edge function.
