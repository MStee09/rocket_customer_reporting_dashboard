/**
 * AISuggestionAssistant - REAL AI VERSION
 * 
 * LOCATION: src/admin/visual-builder/components/AISuggestionAssistant.tsx
 * 
 * This version calls the widget-builder-ai edge function which uses
 * Claude + MCP tools to provide schema-aware suggestions.
 * 
 * NO MORE PATTERN MATCHING - it actually discovers what fields exist
 * and searches for where products/carriers are stored.
 */
...
function buildStepsFromAIResponse(result: any): SuggestionStep[] {
  ...
  // Step 4: AI block if product search needed
  if (result.aiPrompt) {
    steps.push({
      panel: 'logic',
