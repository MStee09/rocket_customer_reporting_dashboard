...
async function fetchWidgetRowData(
  ...
  // Also pass the full filters object so widgets can access carrier_name
  ...
  const widgetDataFetchers: Record<string, () => Promise<{ rows: Record<string, unknown>[]; columns: { key: string; label: string; type: string }[] }>> = {
    ...
    carrier_performance: async () => {
      // Get carrier_name from filters as fallback
      const carrierNameFilter = filters?.carrier_name ? decodeURIComponent(String(filters.carrier_name)) : null;
      
      console.log('[widgetdataservice] carrier_performance called with:', { 
        carrierFilter, 
        carrierNameFilter,
        customerFilter,
        dateRange 
      ...
      // Filter by carrier_id if provided and not 0
      if (carrierFilter && carrierFilter > 0) {
        console.log('[widgetdataservice] Filtering by carrier_id:', carrierFilter);
        query = query.eq('carrier_id', carrierFilter);
      } else if (carrierNameFilter) {
        // Fallback to filtering by carrier_name
        console.log('[widgetdataservice] Filtering by carrier_name:', carrierNameFilter);
