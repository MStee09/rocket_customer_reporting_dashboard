/**
 * Preview Panel - With Customer Selector Support
 *
 * LOCATION: /src/admin/visual-builder/components/panels/PreviewPanel.tsx
 *
 * Features:
 * - Reads previewCustomerId from context (null = all customers)
 * - Uses direct Supabase join for shipment_item queries
 * - Falls back to shipment_report_view for simple queries
 */
...
export function PreviewPanel() {
  ...
  // Check if any logic block requires MCP (filters on joined tables like shipment_item)
  ...
  // Extract product filters from logic blocks
  ...
  // Fetch data with join (for product descriptions)
  const fetchPreviewDataWithJoin = async () => {
    ...
    try {
      ...
      // Build the query with inner join to shipment_item
      let query = supabase
        .from('shipment')
        .select(`
          load_id,
  ...
  // Fetch data directly from shipment_report_view (for simple queries)
  const fetchPreviewDataDirect = async () => {
    ...
    try {
      ...
      // Apply filters from logic blocks
          ...
          <div className="h-full flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-xs text-slate-500 mt-2">
                {usingMCP ? 'Querying with join...' : 'Loading preview...'}
              </p>
            </div>
          </div>
        ) : error ? (
