  ...
  if (!widgetDef) {
    ...
    if (loadedWidgetResult && loadedWidgetResult.widget?.dataSource?.groupByColumn) {
      ...
      if (isMultiDimension) {
        ...
        if (isProductQuery) {
          console.log('[widgetdataservice] Multi-dim product query using mcp_detail');
          ...
          // Build filters for mcp_detail - date filters + description search
          const detailFilters: Array<{field: string; operator: string; value: string}> = [
            { field: 'pickup_date', operator: 'gte', value: dateRange.start },
            { field: 'pickup_date', operator: 'lte', value: dateRange.end }
          ];

          // Add OR filter for search terms (we'll query each term separately and combine)
          ...
          for (const term of searchTerms) {
            ...
            console.log(`[widgetdataservice] Multi-dim mcp_detail querying term: "${term}"`);

            const { data: termResult, error: termError } = await supabase.rpc('mcp_detail', {
              p_table_name: 'shipment_item',
              p_customer_id: effectiveCustomerIdNum || 0,
              p_is_admin: false,
              p_select_columns: null, // Use smart defaults
              p_filters: termFilters,
              p_order_by: 'pickup_date',
              p_order_dir: 'DESC',
              p_limit: 200
            });

            if (termError) {
              console.error(`[widgetdataservice] Multi-dim mcp_detail error for "${term}":`, termError);
              continue;
            }

            const parsed = typeof termResult === 'string' ? JSON.parse(termResult) : termResult;
            if (parsed?.success && parsed?.data) {
              console.log(`[widgetdataservice] Multi-dim "${term}" returned ${parsed.data.length} rows`);
              allItemData.push(...parsed.data);
            } else {
              console.log(`[widgetdataservice] Multi-dim "${term}" returned no data or error:`, parsed?.error);
            }
          }

          detailRows = allItemData;
          console.log('[widgetdataservice] Multi-dim total detail rows:', detailRows.length);

          // Add state data if secondary group is origin_state or destination_state
