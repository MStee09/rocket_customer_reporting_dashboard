...
export async function executeWidget(
  ...
  if (!widgetDef) {
    ...
    if (customWidget && customWidget.dataSource?.groupByColumn) {
      ...
      // Check if this is a multi-dimension widget (has secondary group + search terms)
      ...
      if (isMultiDimension) {
        ...
        // Query each search term separately to get multi-dimension data
        ...
        // Build table data for raw data view - one row per combination
        ...
        // Build grouped chart data for visualization
