...
export async function compileSchemaContext(customerId: string): Promise<SchemaContext> {
  // Try dynamic view first, fall back to table
...
async function fetchSchemaFields(): Promise<SchemaField[]> {
  // Try 1: Dynamic metadata view (includes business context)
  try {
  ...
  // Try 2: Legacy schema_columns table
  try {
    const { data, error } = await supabase
      .from('schema_columns')
    ...
    if (!error && data && data.length > 0) {
      ...
      // Fetch business context separately
