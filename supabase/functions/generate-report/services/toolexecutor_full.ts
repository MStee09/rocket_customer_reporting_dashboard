...
export class ToolExecutor {
  ...
  private async detectAnomalies(input: Record<string, unknown>): Promise<unknown> {
    ...
    try {
      // First, get the data distribution
      ...
      // If we have grouping, analyze per group
      if (groupBy) {
      ...
      // No grouping - return time-based anomalies would need more complex SQL
      return {
        metric,
        baseline_type: baseline,
        sensitivity,
  ...
  private async investigateCause(input: Record<string, unknown>): Promise<unknown> {
    ...
    // Perform multi-dimensional drill-down
  ...
  private async addSection(input: Record<string, unknown>): Promise<unknown> {
    ...
    // Execute section to get preview data
  ...
  private finalizeReport(input: Record<string, unknown>): unknown {
    // Handle both formats: direct report object or using currentReport
  ...
  private generateInsight(input: Record<string, unknown>): unknown {
    ...
    // Generate insight based on data characteristics
