/*
  ...
  1. Changes
    - Added carrier_id from shipment.rate_carrier_id (the correct carrier reference)
    - Added carrier_name from carrier table (joined via rate_carrier_id)
  ...
  3. Notes
    - Uses shipment.rate_carrier_id as the carrier reference (NOT shipment_carrier table)
    - Joins directly to carrier table to get carrier_name
    - Uses LEFT JOIN to handle shipments without assigned carriers
    - All existing columns remain intact
    
  4. Important
    - The shipment_carrier table contains outdated/incorrect carrier assignments
    - Always use shipment.rate_carrier_id for carrier lookups
*/
...
SELECT 
  ...
  -- Use rate_carrier_id from shipment table (this is the correct carrier reference)
  s.rate_carrier_id as carrier_id,
  -- Get carrier_name from carrier table using rate_carrier_id
...
-- Join directly to carrier table using rate_carrier_id (NOT through shipment_carrier)
