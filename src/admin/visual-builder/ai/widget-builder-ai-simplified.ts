/**
 * Widget Builder AI Edge Function - SIMPLIFIED VERSION
 * 
 * LOCATION: supabase/functions/widget-builder-ai/index.ts
 * 
 * This version uses HARDCODED schema knowledge instead of MCP database functions.
 * It's a working fallback until the MCP functions are deployed.
 * 
 * It still uses Claude to interpret the user's prompt, but provides schema
 * context directly in the prompt rather than via tool calls.
 */
...
// Hardcoded schema knowledge - this matches your actual database
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Single API call - no tools needed since schema is in prompt
    ...
    const result = {
      ...
      mcpQuery: {
        // Pass join info for PreviewPanel
