/*
  ...
  Creates an automated anomaly detection system that scans customer shipping data
  for statistical anomalies and automatically populates the detected_anomalies table.
  
  ## New Tables
  - `anomaly_detection_config` - Per-customer configuration for detection thresholds
  
  ## New Functions
  - `get_client_id_for_customer()` - Helper to map customer_id to client_id
  - `run_anomaly_scan_for_customer()` - Scans single customer for anomalies
  - `run_anomaly_scan_all_customers()` - Batch scan all active customers
  - `get_anomaly_dashboard_summary()` - Customer-facing anomaly summary
  - `get_admin_anomaly_summary()` - Admin view of all customer anomalies
  - `cleanup_old_anomalies()` - Removes old resolved/dismissed anomalies
  
  ## Anomaly Types Detected
  - spend_spike / spend_drop - Daily spend anomalies
  - volume_spike / volume_drop - Shipment count anomalies
  - concentration_risk - Over-reliance on single carrier
  - new_lane - First shipments to new origin/destination pairs
  
  ## Security
  - Enable RLS on anomaly_detection_config
  - Admin-only management of config
  - Service role can execute scan functions
...
CREATE TABLE IF NOT EXISTS anomaly_detection_config (
  ...
  concentration_risk_threshold NUMERIC DEFAULT 60.0,
...
BEGIN
  ...
  IF v_config.detect_lane_anomalies THEN
    ...
    -- Get row count and add to anomalies found
