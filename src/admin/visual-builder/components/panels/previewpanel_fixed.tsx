/**
 * PreviewPanel - Fixed version with proper AI Logic block detection
 * 
 * LOCATION: src/admin/visual-builder/components/panels/PreviewPanel.tsx
 * 
 * Fixes:
 * 1. Properly detects AI Logic blocks with compiledRule
 * 2. Extracts product filters from both Filter blocks AND AI Logic blocks
 * 3. Includes customer selector in the preview header
 */
...
export function PreviewPanel() {
  ...
  // ==========================================================================
  // HELPER: Check if we need a join query (product descriptions, etc.)
  // ==========================================================================
      ...
      if (block.type === 'filter') {
        ...
        for (const condition of filterBlock.conditions || []) {
          ...
          if (
            ...
            console.log('[PreviewPanel] needsJoin: true (filter block field:', field, ')');
            return true;
          }
        }
      }

      // Check AI Logic blocks with compiled rules
      if (block.type === 'ai') {
        ...
        if (aiBlock.compiledRule?.filters) {
          for (const filter of aiBlock.compiledRule.filters) {
            ...
            if (
              ...
              console.log('[PreviewPanel] needsJoin: true (AI block field:', field, ')');
              return true;
            }
          }
        }
  ...
  // ==========================================================================
  // HELPER: Extract product filter values from all logic blocks
  // ==========================================================================
  ...
  // ==========================================================================
  // FETCH: With join query for product descriptions
  // ==========================================================================
  const fetchPreviewDataWithJoin = async () => {
    ...
    try {
      ...
      // Build the query with join to shipment_item
      let query = supabase
        .from('shipment')
        .select(`
          load_id,
      ...
      // Apply customer filter if not "all customers"
      ...
      // Client-side filter for products (OR logic)
  ...
  // ==========================================================================
  // FETCH: Direct query from shipment_report_view
  // ==========================================================================
  const fetchPreviewDataDirect = async () => {
    ...
    try {
      ...
      // Apply filters from logic blocks (non-description filters)
      for (const block of state.logicBlocks || []) {
        if (!block.enabled) continue;
        ...
        if (block.type === 'filter') {
          ...
          for (const condition of filterBlock.conditions || []) {
            ...
            // Skip join-only fields
            if (field.includes('shipment_item') || field.includes('.')) continue;
  ...
  const fetchPreviewData = async () => {
    ...
    console.log('[PreviewPanel] Starting fetch - needsJoin:', needsJoin);
  ...
  return (
    <div className={containerClass}>
      ...
      {/* Chart Area */}
      <div className={`mx-4 bg-slate-50 rounded-lg ${isFullscreen ? 'h-[calc(100vh-180px)]' : 'h-80'}`}>
        {loading ? (
...
function processDataForVisualization(rawData: any[], config: any, productFilters: string[]): any[] {
  ...
  // For product queries - group by product category
  if (productFilters.length > 0 && rawData.length > 0 && rawData[0]?.description) {
