...
export function DashboardWidgetCard({
  ...
  // Check if this is a Visual Builder widget with dataSource config
    ...
    queryFn: async () => {
      if (isCustomWidget) {
        ...
        const normalizeChartData = (data: any[]): any[] => {
          ...
          const first = data[0];
          if ('name' in first && 'value' in first) return data;
          if ('label' in first && 'value' in first) {
            return data.map(item => ({ name: String(item.label ?? ''), value: Number(item.value) || 0 }));
          }
          ...
          if (keys.length >= 2) {
            ...
            return data.map(item => ({ name: String(item[nameKey] ?? ''), value: Number(item[valueKey]) || 0 }));
          }
          return data;
        };

        // DYNAMIC QUERY: If this is a Visual Builder widget, query fresh data
        if (isVisualBuilderWidget) {
          console.log('[DashboardWidgetCard] Running dynamic query for widget:', widget.id);
          
          const { groupByColumn, metricColumn, aggregation, filters, aiConfig } = widget.dataSource;
          
          // Determine table based on column
          ...
          // Add product filters from aiConfig if present
          ...
          console.log('[DashboardWidgetCard] Query returned:', rows.length, 'rows');
          ...
          const chartData = rows.map((row: any) => {
            ...
            const valueKey = Object.keys(row).find(k => 
              k.includes(aggregation || 'avg') || k === 'value' || (typeof row[k] === 'number' && k !== labelKey)
            );
            return {
        ...
        if (widget.dataMode === 'static' && widget.snapshotData) {
          ...
          return { data: normalizeChartData(snapshot.data || []), type: snapshot.type || 'chart' };
        }

        // Fallback: visualization.data
        if (widget.visualization?.data && Array.isArray(widget.visualization.data)) {
          return { data: normalizeChartData(widget.visualization.data), type: widget.type || 'chart' };
        }
  ...
  const renderContent = () => {
    ...
    // Table view for Visual Builder widgets
    if (isVisualBuilderWidget && showTableView && data.data) {
      const tableData = data.data;
      const metricLabel = widget.dataSource?.metricColumn || 'Value';
      const groupLabel = widget.dataSource?.groupByColumn || 'Category';
      const aggregation = widget.dataSource?.aggregation?.toUpperCase() || 'AVG';
      const isCurrency = metricLabel.includes('retail') || metricLabel.includes('cost') || metricLabel.includes('margin');
            ...
            <thead className="bg-slate-50 sticky top-0">
              <tr>
                <th className="text-left px-3 py-2 font-medium text-slate-600 border-b">
                  {groupLabel.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                </th>
                <th className="text-right px-3 py-2 font-medium text-slate-600 border-b">
                  {aggregation} {metricLabel.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                </th>
              </tr>
            </thead>
            <tbody>
              {tableData.map((row: any, idx: number) => (
                <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-3 py-2 text-right text-slate-700">
                    {isCurrency ? `$${row.value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : row.value.toLocaleString()}
                  </td>
                </tr>
              ))}
            </tbody>
    ...
    >
      <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 flex-shrink-0">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {/* Table view toggle for Visual Builder widgets */}
          {isVisualBuilderWidget && !isLoading && !error && data && (
            <button
              onClick={(e) => { e.stopPropagation(); setShowTableView(!showTableView); }}
              className={`p-1.5 rounded-lg hover:bg-slate-100 transition-colors ${showTableView ? 'text-blue-600 bg-blue-50' : 'text-slate-400 hover:text-slate-600'}`}
              title={showTableView ? "Show chart" : "Show data table"}
            >
