...
function parseUserIntent(prompt: string): AISuggestion {
  ...
  // Detect fields - USE CORRECT SCHEMA FIELD NAMES
  let xField = '';
  let yField = '';
  let warning = '';

  // X-Axis / grouping field detection
  if (lower.includes('carrier')) {
    ...
    // IMPORTANT: Product descriptions are NOT in shipment_report_view
    // The field 'description' exists only in shipment_item table
    xField = 'reference_number'; // Fallback to available field
  ...
  // Detect product filtering - USE SET TO DEDUPLICATE
  ...
  if (uniqueProducts.length > 0) {
    ...
    // Note: This filter references a field that may not exist
    // The warning will inform the user
    filters.push({
      field: 'item_descriptions', // Correct field name after migration
