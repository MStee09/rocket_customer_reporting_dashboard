# PHASE 5: CONTEXT SERVICE (Code Cleanup)

## Time: ~15 minutes
## Risk: Low
## Impact: Removes ~180 lines from main file, improves testability

---

## PASTE 1 of 2: Create Context Service

**Instruction for Bolt:**
> Create new file `supabase/functions/generate-report/services/contextService.ts` with this content:
...
## PASTE 2 of 2: Update Edge Function to Use Context Service
...
> ```
>
> 2. DELETE these functions (approximately lines 360-546):
...
>    
>    (Keep `getAccessControlPrompt` if you didn't already delete it in Phase 1)
>
> 3. REPLACE lines 742-750 (the context compilation block) FROM:
> ```typescript
...
>     
>     const schemaPrompt = context.prompts.schema;
>     const knowledgePrompt = context.prompts.knowledge;
>     const profilePrompt = context.prompts.profile;
>     const accessPrompt = context.prompts.access;
>     const availableFieldNames = context.availableFieldNames;
> ```
...
**What this does:**
- Extracts ~180 lines of context compilation into a separate service
- Main file is now ~870 lines instead of ~1050
- Context compilation can be unit tested independently

**Test by:**
1. Create an AI report
2. Verify schema fields, knowledge, and customer profile are still loaded correctly
3. Check that restricted fields are still hidden from customers
...
## Ready for Phase 6?
Phase 6 extracts tool execution into a separate service (another ~150 lines removed).
